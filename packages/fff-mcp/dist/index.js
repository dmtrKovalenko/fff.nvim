...
import { fileSuffix, formatGrepResults } from "./output";
...
function parseArgs(argv) {
    ...
    if (!frecencyDbPath || !historyDbPath) {
        const home = homedir();
        const nvimCacheDir = platform() === "win32"
            ? join(home, "AppData", "Local", "nvim-data")
            : join(home, ".cache", "nvim");
        const nvimDataDir = platform() === "win32"
            ? join(home, "AppData", "Local", "nvim-data")
...
async function main() {
    ...
    server.registerTool("find_files", {
        ...
        description: "Fuzzy file search by name. Use this first to discover file names and directory structure before grepping content. " +
    ...
    function performGrep(pattern, mode, maxResults = 50, cursorId, outputMode = "content", context) {
        ...
        const text = formatGrepResults(result.value, outputMode, maxResults, regexFallbackError);
        return {
    ...
    server.registerTool("grep", {
        title: "FFF Grep",
        description: "Search file contents for exact literal text. " +
        ...
        inputSchema: {
            pattern: z
                .string()
                .describe("Exact search text with optional constraint prefixes (e.g. '*.ts MyFunction')."),
            maxResults: z
                .number()
                .optional()
            ...
            cursor: z
                .string()
                .optional()
                .describe("Cursor from previous result. Only use if previous results weren't sufficient."),
            output_mode: z
    ...
    }, async ({ pattern, maxResults = 50, cursor: cursorId, output_mode: outputMode = "content", context, }) => {
    ...
    server.registerTool("regex_grep", {
        ...
        description: "Search file contents with regex patterns (e.g. 'fn\\s+\\w+', 'import.*from'). " +
            "Falls back to literal match if regex fails to compile. See server instructions for constraint syntax.",
        inputSchema: {
            pattern: z
                .string()
                .describe("Regex pattern with optional constraint prefixes (e.g. '*.rs fn\\s+\\w+')."),
            maxResults: z
                .number()
                .optional()
                .describe("Max matching lines (default 50)."),
    ...
    server.registerTool("multi_grep", {
        ...
        description: "Search file contents for lines matching ANY of multiple patterns (OR logic). " +
        ...
        inputSchema: {
            ...
            constraints: z
                .string()
                .optional()
                .describe("File constraints (e.g. '*.{ts,tsx} !test/'). ALWAYS provide when possible."),
            maxResults: z
                .number()
                .optional()
                .describe("Max matching lines (default 50)."),
    ...
    }, async ({ patterns, constraints, maxResults = 50, cursor: cursorId, output_mode: outputMode = "content", context, }) => {
        const nativeCursor = cursorId ? (getCursor(cursorId) ?? null) : null;
