...
fn parse_number_pair(location: &str, split_char: char) -> Option<(i32, i32)> {
    ...
    let start = i32::from_str_radix(start_str, 10).ok()?;
    let end = i32::from_str_radix(end_str, 10).ok()?;
...
fn parse_column_range(start_part: &str, end_part: &str) -> Option<Location> {
    ...
    let line = i32::from_str_radix(line_str, 10).ok()?;
    let start_col = i32::from_str_radix(start_col_str, 10).ok()?;
    let end_col = i32::from_str_radix(end_part, 10).ok()?;
...
fn try_parse_column_position(location: &str) -> Option<Location> {
    ...
    let line = i32::from_str_radix(line_str, 10).ok()?;
    let col = i32::from_str_radix(col_str, 10).ok()?;

    Some(Location::Position { line, col })
}

/// Parses various location formats like file:12, file:12:4, file:12-114
fn parse_column_location<'a>(query: &'a str) -> Option<(&'a str, Location)> {
    ...
    if let Some(line_location) = i32::from_str_radix(location_part, 10).ok() {
...
fn parse_vstudio_location<'a>(query: &'a str) -> Option<(&'a str, Location)> {
    if !query.ends_with(')') {
        return None;
    }
    ...
    if let Ok(line) = i32::from_str_radix(location, 10) {
...
pub fn parse_location<'a>(query: &'a str) -> (&'a str, Option<Location>) {
    // simply ignore the last semicolon even if ther are no additional location info
    let query = query.trim_end_matches(|c| c == ':' || c == '-' || c == '(');
