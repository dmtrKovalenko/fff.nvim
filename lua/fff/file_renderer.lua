...
function M.apply_highlights(item, ctx, item_idx, buf, ns_id, line_idx, line_content)
  ...
  if icon and icon_hl_group and vim.fn.strdisplaywidth(icon) > 0 then
    ...
    vim.api.nvim_buf_set_extmark(buf, ns_id, line_idx - 1, 0, { end_col = vim.fn.strdisplaywidth(icon), hl_group = icon_hl })
  end
  ...
  if ctx.config.git and ctx.config.git.status_text_color and icon and #filename > 0 then
    ...
    if git_text_hl and git_text_hl ~= '' and not is_current_file then
      ...
      vim.api.nvim_buf_set_extmark(buf, ns_id, line_idx - 1, filename_start, { end_col = filename_start + #filename, hl_group = git_text_hl })
    end
  end
  ...
  if ctx.debug_enabled then
    ...
    if start_pos and end_pos then
      vim.api.nvim_buf_set_extmark(buf, ns_id, line_idx - 1, start_pos - 1, { end_col = end_pos, hl_group = ctx.config.hl.frecency })
    end
  end
