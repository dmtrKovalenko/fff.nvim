...
local function verify_sha256(file_path, expected_hash)
  local cmd
  ...
  if result.code ~= 0 then return false, 'sha256 command failed: ' .. (result.stderr or 'unknown error') end

  -- All three tools print the hash as the first whitespace-delimited token
  local actual_hash = (result.stdout or ''):match('^%s*([0-9a-fA-F]+)')
  if not actual_hash then return false, 'Could not parse sha256 output: ' .. tostring(result.stdout) end

  if actual_hash:lower() ~= expected_hash:lower() then
    return false, string.format('SHA256 mismatch: expected %s, got %s', expected_hash, actual_hash:lower())
  end
...
local function download_from_github(version, binary_path, opts, callback)
  ...
  download_file(sha_url, tmp_sha_path, { proxy = opts.proxy }, function(sha_success, sha_err)
    ...
    download_file(url, tmp_path, {
      ...
      local hash_ok, hash_err = verify_sha256(tmp_path, expected_hash)
      if not hash_ok then
        vim.uv.fs_unlink(tmp_path)
