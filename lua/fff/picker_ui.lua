...
--- @param callback function Function called with results: function(results, metadata, location) -> boolean
...
function M.open_with_callback(query, callback, opts)
  ...
  if type(callback) == 'function' then
    local ok, result = pcall(callback, results, metadata, location)
    if ok then
