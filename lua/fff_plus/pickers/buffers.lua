...
function M.get_listed_buffers()
  ...
  for bufnr = 1, vim.fn.bufnr('$') do
    if vim.fn.buflisted(bufnr) == 1 then
      ...
      if buftype ~= 'quickfix' then table.insert(buffers, bufnr) end
    end
  end
  return buffers
end
...
function M.format_buffer(bufnr)
  ...
  local flags = ''
  if modified then flags = flags .. '[+]' end
  if readonly then flags = flags .. '[RO]' end
...
function M.select(action)
  ...
  if item.line and item.line > 0 then pcall(vim.api.nvim_win_set_cursor, 0, { item.line, 0 }) end
end
...
function M.open(opts)
  ...
  if merged_config.title == nil then merged_config.title = 'Buffers' end
