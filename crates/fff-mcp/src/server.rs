...
use fff_core::grep::{self, has_regex_metacharacters, GrepMode, GrepSearchOptions};
...
use crate::output::{file_suffix, format_grep_results, FormatGrepParams, OutputMode};
        ...
        if result.matches.is_empty() && file_offset == 0 {
            ...
            if parts.len() >= 2 {
                ...
                if !is_valid_constraint {
                    ...
                    let retry_parsed = parser.parse(&rest_query);
                    ...
                    if !retry_result.matches.is_empty() && retry_result.matches.len() <= 10 {
                        let mut cs = self.cursor_store.lock().unwrap();
                        let text = format_grep_results(
                            &FormatGrepParams {
        ...
        let (items, scores, total_matched) =
            if result.items.is_empty() && words.len() >= 3 && page_offset == 0 {
                if let Some(shorter) = &shorter {
                    let shorter_query = parser.parse(query);
                    let retry =
