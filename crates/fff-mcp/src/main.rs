...
pub const MCP_INSTRUCTIONS: &str = concat!(
    ...
    "  ❌ 'load.*metadata.*InProgressQuote' → regex spanning multiple tokens, 0 results\n",
    "  ❌ 'ctx.data::<ActorAuth>'     → code syntax, too specific, 0 results\n",
    "  ❌ 'struct ActorAuth'          → adding keywords narrows results, misses enums/traits/type aliases\n",
    "  ❌ 'TODO.*#\\d+'               → complex regex, use simple 'TODO' then filter visually\n",
    "\n",
    ...
    "  ❌ grep 'ActorAuth' → grep 'PopulatedActorAuth' → grep 'actor_auth'  (3 calls wasted)\n",
    "\n",
    "## Workflow\n",
    "\n",
    ...
    "  ❌ 'quote TODO'     → searches for literal text 'quote TODO', finds nothing\n",
    "\n",
    "Prefer broad constraints:\n",
    "  ✅ '*.rs query'           → file type\n",
    "  ✅ 'quotes/ query'        → top-level dir\n",
    "  ❌ 'quotes/storage/db/ query' → too specific, misses results\n",
    "\n",
    "## Output Format\n",
    "\n",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let scan_picker = shared_picker.clone();
    tokio::task::spawn_blocking(move || {
        let start = std::time::Instant::now();
        loop {
            let is_scanning = scan_picker
                .read()
                .ok()
