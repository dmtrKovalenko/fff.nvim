...
pub unsafe extern "C" fn fff_live_grep(
    ...
    let parsed = if is_ai {
        fff_core::grep::parse_ai_grep_query(query_str)
    } else {
    ...
    let result = fff_core::grep::grep_search(picker.get_files(), query_str, parsed.as_ref(), &options);
...
pub unsafe extern "C" fn fff_multi_grep(
    ...
    let parsed_constraints = opts.constraints.as_deref().and_then(|c| {
        if is_ai {
            fff_core::grep::parse_ai_grep_query(c)
        } else {
