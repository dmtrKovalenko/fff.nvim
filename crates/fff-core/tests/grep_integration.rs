//! Integration tests for grep functionality against real filesystem.
//!
//! These tests create temporary directories with known file contents,
//! build `FileItem` slices, and run `grep_search` / `grep_count` to
//! verify correctness of both PlainText and Regex modes.

use std::fs;
use std::path::Path;
use tempfile::TempDir;

use fff_core::grep::{GrepMode, GrepSearchOptions, grep_count, grep_search, parse_grep_query};
...
#[test]
fn plain_text_count_matches_basic() {
    let tmp = TempDir::new().unwrap();
    let files = vec![
        create_file(tmp.path(), "a.txt", "foo\nbar\nfoo\n"),
        create_file(tmp.path(), "b.txt", "baz\nfoo\n"),
    ];

    let (count, files_searched) = grep_count(&files, "foo", &plain_opts());
...
#[test]
fn regex_count_basic() {
    let tmp = TempDir::new().unwrap();
    let files = vec![
        create_file(tmp.path(), "a.txt", "foo123\nbar456\nfoo789\n"),
        create_file(tmp.path(), "b.txt", "nope\nfoo000\n"),
    ];

    let (count, files_searched) = grep_count(&files, "foo\\d+", &regex_opts());
...
#[test]
fn plain_text_and_regex_count_agree_on_literal() {
    let tmp = TempDir::new().unwrap();
    let files = vec![
        create_file(tmp.path(), "a.txt", "apple\nbanana\napple pie\n"),
        create_file(tmp.path(), "b.txt", "apple cider\ngrape\n"),
    ];

    let (plain_count, _) = grep_count(&files, "apple", &plain_opts());
    let (regex_count, _) = grep_count(&files, "apple", &regex_opts());

    assert_eq!(plain_count, regex_count);
