...
use fff_core::grep::{GrepSearchOptions, grep_count, grep_search, parse_grep_query};
use std::io::Read;
use std::path::Path;
...
/// fff count-only: raw search engine speed, no match collection.
fn run_fff_count(files: &[FileItem], query: &str) -> (usize, Duration) {
    ...
    let start = Instant::now();
    let (count, _files) = grep_count(files, query, &options);
    let elapsed = start.elapsed();
    (count, elapsed)
}
...
fn main() {
    ...
    // ── Section 1: Count-only (raw engine speed, N iterations) ─────────

    eprintln!(
        "[3/5] Raw search engine: fff count-only vs rg --count-matches ({} iters, showing min)\n",
        iters
    );
    eprintln!(
        "  {:<22} | {:>9} {:>10} | {:>9} {:>10} | {:>7}",
    ...
    let mut fff_count_total = Duration::ZERO;
    let mut rg_count_total = Duration::ZERO;

    for (name, query, ci) in &queries {
        let q = *query;
        let ci = *ci;
        let fs = run_n(|| run_fff_count(&files, q), iters);
        let rs = run_n(|| run_rg_count(&canonical, q, ci, threads), iters);
        ...
        fff_count_total += fs.min;
        rg_count_total += rs.min;
    }

    eprintln!(
        "  {:<22} | {:>9} {:>10} | {:>9} {:>10} | {:>7}",
        "TOTAL",
        fmt_dur(fff_count_total),
        "",
        fmt_dur(rg_count_total),
        "",
        ratio_str(fff_count_total, rg_count_total),
    );

    // ── Section 2: Full collection (N iterations) ──────────────────────
    ...
    // ── Section 3: First-page / UI scenario (N iterations) ─────────────
